...
class KeyManager : public RefCountInterface {
 public:
  ...
  virtual bool SetKey(const std::string participant_id, int index, vector<uint8_t> key) = 0;

  /// Set the keys.
  virtual bool SetKeys(const std::string participant_id, vector<vector<uint8_t>> keys) = 0;

  /// Get the keys.
  virtual const vector<vector<uint8_t>> GetKeys(const std::string participant_id) const = 0;
...
class RTCFrameCryptor : public RefCountInterface {
 public:
  ...
  virtual const std::string participant_id() const = 0;
