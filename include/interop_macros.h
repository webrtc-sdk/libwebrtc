...
#define DECLARE_GET_STRING(handle, value, sz_value, class_name, fun_name) \
  ...
  libwebrtc::scoped_refptr<class_name> p = static_cast<class_name*>(handle);         \
...
#define DECLARE_GET_LIST_HANDLE(handle, value, value_type_name,               \
  ...
  CHECK_NATIVE_HANDLE(handle);                                                \
  libwebrtc::scoped_refptr<owner_class_name> p = static_cast<owner_class_name*>(handle); \
  libwebrtc::scoped_refptr<list_class_name> pList =                                      \
...
#define DECLARE_SET_LIST_HANDLE(handle, value, owner_class_name,               \
  ...
  CHECK_NATIVE_HANDLE(handle);                                                 \
  libwebrtc::scoped_refptr<owner_class_name> pOwner =                                     \
      static_cast<owner_class_name*>(handle);                                  \
  libwebrtc::scoped_refptr<list_class_name> pList = static_cast<list_class_name*>(value); \
  pOwner->fun_name(pList->to_vector());                                        \
  return rtcResultU4::kSuccess;

#define DECLARE_LIST_GET_COUNT(handle, list_type_name)                        \
  CHECK_POINTER_EX(handle, 0);                                                \
  libwebrtc::scoped_refptr<list_type_name> pList = static_cast<list_type_name*>(handle); \
...
#define DECLARE_LIST_GET_ITEM(handle, index, out_val, out_val_type_name,      \
  ...
  libwebrtc::scoped_refptr<item_type_name> pItem =                                       \
