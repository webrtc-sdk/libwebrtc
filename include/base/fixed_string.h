// Customise the behaviour of fixed_string by defining these before including it:
...
// A fixed-sized string that can store N characters (excluding the null-terminator)
// E.g., fixed_string<4> has a buffer of 5 characters
// Main use is to store small constant strings.
template<int Capacity> class fixed_string {
...
public:
    ...
    explicit fixed_string(bool truncates):size_(0), truncates_(truncates) { zero_contents(); data_[0] = '\0'; }
...
template<int M> std::ostream& operator<<(std::ostream& out, const fixed_string<M>& str) {
