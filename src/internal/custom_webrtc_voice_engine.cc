...
#include "custom_webrtc_voice_engine.h"
...
webrtc::AudioReceiveStreamInterface::Config BuildReceiveStreamConfig(
    uint32_t remote_ssrc, uint32_t local_ssrc, bool use_nack,
    bool enable_non_sender_rtt, const std::vector<std::string>& stream_ids,
    const std::vector<webrtc::RtpExtension>& extensions,
    webrtc::Transport* rtcp_send_transport,
    const rtc::scoped_refptr<webrtc::AudioDecoderFactory>& decoder_factory,
    const std::map<int, webrtc::SdpAudioFormat>& decoder_map,
    absl::optional<webrtc::AudioCodecPairId> codec_pair_id,
    size_t jitter_buffer_max_packets, bool jitter_buffer_fast_accelerate,
...
CustomWebRtcVoiceEngine::CustomWebRtcVoiceEngine(
  ...
  RTC_LOG(LS_INFO) << "CustomWebRtcVoiceEngine::CustomWebRtcVoiceEngine";
...
void CustomWebRtcVoiceEngine::Init() {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  RTC_LOG(LS_INFO) << "CustomWebRtcVoiceEngine::Init";
  ...
  // Set up AudioState.
  {
    ...
    audio_state_ = create_audio_state_factory_->CreateAudioState(config);
  }
...
rtc::scoped_refptr<webrtc::AudioState> CustomWebRtcVoiceEngine::GetAudioState()
    const {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  return audio_state_;
}
...
CustomWebRtcVoiceEngine::CreateSendChannel(
    webrtc::Call* call, const MediaConfig& config, const AudioOptions& options,
    const webrtc::CryptoOptions& crypto_options,
    webrtc::AudioCodecPairId codec_pair_id) {
  return std::make_unique<WebRtcVoiceSendChannel2>(
      this, config, options, crypto_options, call, codec_pair_id);
}

std::unique_ptr<VoiceMediaReceiveChannelInterface>
CustomWebRtcVoiceEngine::CreateReceiveChannel(
    webrtc::Call* call, const MediaConfig& config, const AudioOptions& options,
    const webrtc::CryptoOptions& crypto_options,
    webrtc::AudioCodecPairId codec_pair_id) {
  return std::make_unique<WebRtcVoiceReceiveChannel2>(
      this, config, options, crypto_options, call, codec_pair_id);
}

void CustomWebRtcVoiceEngine::ApplyOptions(const AudioOptions& options_in) {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  RTC_LOG(LS_INFO) << "CustomWebRtcVoiceEngine::ApplyOptions: "
                   << options_in.ToString();
...
const std::vector<AudioCodec>& CustomWebRtcVoiceEngine::send_codecs() const {
  RTC_DCHECK(signal_thread_checker_.IsCurrent());
  return send_codecs_;
}

const std::vector<AudioCodec>& CustomWebRtcVoiceEngine::recv_codecs() const {
  RTC_DCHECK(signal_thread_checker_.IsCurrent());
  return recv_codecs_;
}

std::vector<webrtc::RtpHeaderExtensionCapability>
CustomWebRtcVoiceEngine::GetRtpHeaderExtensions() const {
  RTC_DCHECK(signal_thread_checker_.IsCurrent());
...
bool CustomWebRtcVoiceEngine::StartAecDump(webrtc::FileWrapper file,
...
void CustomWebRtcVoiceEngine::StopAecDump() {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  webrtc::AudioProcessing* ap = apm();
  if (ap) {
    ap->DetachAecDump();
...
CustomWebRtcVoiceEngine::GetAudioDeviceStats() {
  return adm()->GetStats();
}

webrtc::AudioDeviceModule* CustomWebRtcVoiceEngine::adm() {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  RTC_DCHECK(adm_);
  return adm_.get();
}

webrtc::AudioProcessing* CustomWebRtcVoiceEngine::apm() const {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  return apm_.get();
}

webrtc::AudioState* CustomWebRtcVoiceEngine::audio_state() {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  RTC_DCHECK(audio_state_);
  return audio_state_.get();
}

std::vector<AudioCodec> CustomWebRtcVoiceEngine::CollectCodecs(
...
// --------------------------------- WebRtcVoiceSendChannel2 ------------------

class WebRtcVoiceSendChannel2::WebRtcAudioSendStream
    : public AudioSource::Sink {
 public:
  WebRtcAudioSendStream(
      uint32_t ssrc, const std::string& mid, const std::string& c_name,
      const std::string track_id,
      const absl::optional<webrtc::AudioSendStream::Config::SendCodecSpec>&
          send_codec_spec,
      bool extmap_allow_mixed,
      const std::vector<webrtc::RtpExtension>& extensions,
      int max_send_bitrate_bps, int rtcp_report_interval_ms,
      const absl::optional<std::string>& audio_network_adaptor_config,
      webrtc::Call* call, webrtc::Transport* send_transport,
      const rtc::scoped_refptr<webrtc::AudioEncoderFactory>& encoder_factory,
...
WebRtcVoiceSendChannel2::WebRtcVoiceSendChannel2(
    CustomWebRtcVoiceEngine* engine, const MediaConfig& config,
    const AudioOptions& options, const webrtc::CryptoOptions& crypto_options,
    webrtc::Call* call, webrtc::AudioCodecPairId codec_pair_id)
  ...
  RTC_LOG(LS_VERBOSE) << "WebRtcVoiceSendChannel2::WebRtcVoiceSendChannel2";
  RTC_DCHECK(call);
  SetOptions(options);
}
...
WebRtcVoiceSendChannel2::~WebRtcVoiceSendChannel2() {
  ...
  RTC_DLOG(LS_VERBOSE) << "WebRtcVoiceSendChannel2::~WebRtcVoiceSendChannel2";
...
bool WebRtcVoiceSendChannel2::SetOptions(const AudioOptions& options) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
bool WebRtcVoiceSendChannel2::SetSenderParameters(
...
absl::optional<Codec> WebRtcVoiceSendChannel2::GetSendCodec() const {
  if (send_codec_spec_) {
...
bool WebRtcVoiceSendChannel2::SetSendCodecs(
    const std::vector<Codec>& codecs, absl::optional<Codec> preferred_codec) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
void WebRtcVoiceSendChannel2::SetSend(bool send) {
...
bool WebRtcVoiceSendChannel2::SetAudioSend(uint32_t ssrc, bool enable,
                                           const AudioOptions* options,
                                           AudioSource* source) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
bool WebRtcVoiceSendChannel2::AddSendStream(const StreamParams& sp) {
...
bool WebRtcVoiceSendChannel2::RemoveSendStream(uint32_t ssrc) {
...
void WebRtcVoiceSendChannel2::SetSsrcListChangedCallback(
    absl::AnyInvocable<void(const std::set<uint32_t>&)> callback) {
  ssrc_list_changed_callback_ = std::move(callback);
}

bool WebRtcVoiceSendChannel2::SetLocalSource(uint32_t ssrc,
                                             AudioSource* source) {
  auto it = send_streams_.find(ssrc);
  if (it == send_streams_.end()) {
    if (source) {
...
bool WebRtcVoiceSendChannel2::CanInsertDtmf() {
  return dtmf_payload_type_.has_value() && send_;
}

void WebRtcVoiceSendChannel2::SetFrameEncryptor(
    uint32_t ssrc,
    rtc::scoped_refptr<webrtc::FrameEncryptorInterface> frame_encryptor) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
bool WebRtcVoiceSendChannel2::InsertDtmf(uint32_t ssrc, int event,
                                         int duration) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
void WebRtcVoiceSendChannel2::OnPacketSent(const rtc::SentPacket& sent_packet) {
...
void WebRtcVoiceSendChannel2::OnNetworkRouteChanged(
    absl::string_view transport_name, const rtc::NetworkRoute& network_route) {
  RTC_DCHECK_RUN_ON(&network_thread_checker_);
...
bool WebRtcVoiceSendChannel2::MuteStream(uint32_t ssrc, bool muted) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
bool WebRtcVoiceSendChannel2::SetMaxSendBitrate(int bps) {
...
void WebRtcVoiceSendChannel2::OnReadyToSend(bool ready) {
  RTC_DCHECK_RUN_ON(&network_thread_checker_);
...
bool WebRtcVoiceSendChannel2::GetStats(VoiceMediaSendInfo* info) {
...
void WebRtcVoiceSendChannel2::FillSendCodecStats(
...
void WebRtcVoiceSendChannel2::SetEncoderToPacketizerFrameTransformer(
    uint32_t ssrc,
    rtc::scoped_refptr<webrtc::FrameTransformerInterface> frame_transformer) {
  RTC_DCHECK_RUN_ON(worker_thread_);
  auto matching_stream = send_streams_.find(ssrc);
...
webrtc::RtpParameters WebRtcVoiceSendChannel2::GetRtpSendParameters(
    uint32_t ssrc) const {
  RTC_DCHECK_RUN_ON(worker_thread_);
  auto it = send_streams_.find(ssrc);
  if (it == send_streams_.end()) {
...
webrtc::RTCError WebRtcVoiceSendChannel2::SetRtpSendParameters(
...
// -------------------------- WebRtcVoiceReceiveChannel2 ----------------------

class WebRtcVoiceReceiveChannel2::WebRtcAudioReceiveStream {
 public:
  ...
  bool SetBaseMinimumPlayoutDelayMs(int delay_ms) {
    RTC_DCHECK_RUN_ON(&worker_thread_checker_);
    if (stream_->SetBaseMinimumPlayoutDelayMs(delay_ms)) return true;
...
WebRtcVoiceReceiveChannel2::WebRtcVoiceReceiveChannel2(
    CustomWebRtcVoiceEngine* engine, const MediaConfig& config,
    const AudioOptions& options, const webrtc::CryptoOptions& crypto_options,
    webrtc::Call* call, webrtc::AudioCodecPairId codec_pair_id)
    : MediaChannelUtil(call->network_thread(), config.enable_dscp),
...
bool WebRtcVoiceReceiveChannel2::SetReceiverParameters(
...
webrtc::RtpParameters WebRtcVoiceReceiveChannel2::GetRtpReceiverParameters(
    uint32_t ssrc) const {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
WebRtcVoiceReceiveChannel2::GetDefaultRtpReceiveParameters() const {
  RTC_DCHECK_RUN_ON(worker_thread_);
  webrtc::RtpParameters rtp_params;
  if (!default_sink_) {
...
bool WebRtcVoiceReceiveChannel2::SetOptions(const AudioOptions& options) {
  RTC_DCHECK_RUN_ON(worker_thread_);
  RTC_LOG(LS_INFO) << "Setting voice channel options: " << options.ToString();
...
bool WebRtcVoiceReceiveChannel2::SetRecvCodecs(
...
void WebRtcVoiceReceiveChannel2::SetReceiveNackEnabled(bool enabled) {
...
void WebRtcVoiceReceiveChannel2::SetReceiveNonSenderRttEnabled(bool enabled) {
...
void WebRtcVoiceReceiveChannel2::SetPlayout(bool playout) {
...
bool WebRtcVoiceReceiveChannel2::AddRecvStream(const StreamParams& sp) {
...
bool WebRtcVoiceReceiveChannel2::RemoveRecvStream(uint32_t ssrc) {
...
void WebRtcVoiceReceiveChannel2::ResetUnsignaledRecvStream() {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
absl::optional<uint32_t> WebRtcVoiceReceiveChannel2::GetUnsignaledSsrc() const {
...
void WebRtcVoiceReceiveChannel2::ChooseReceiverReportSsrc(
...
// receiving.
void WebRtcVoiceReceiveChannel2::OnDemuxerCriteriaUpdatePending() {}
void WebRtcVoiceReceiveChannel2::OnDemuxerCriteriaUpdateComplete() {}

bool WebRtcVoiceReceiveChannel2::SetOutputVolume(uint32_t ssrc, double volume) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
bool WebRtcVoiceReceiveChannel2::SetDefaultOutputVolume(double volume) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
bool WebRtcVoiceReceiveChannel2::SetBaseMinimumPlayoutDelayMs(uint32_t ssrc,
                                                              int delay_ms) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
absl::optional<int> WebRtcVoiceReceiveChannel2::GetBaseMinimumPlayoutDelayMs(
    uint32_t ssrc) const {
  // SSRC of 0 represents the default receive stream.
  if (ssrc == 0) {
...
void WebRtcVoiceReceiveChannel2::SetFrameDecryptor(
    uint32_t ssrc,
    rtc::scoped_refptr<webrtc::FrameDecryptorInterface> frame_decryptor) {
  RTC_DCHECK_RUN_ON(worker_thread_);
...
void WebRtcVoiceReceiveChannel2::OnPacketReceived(
  ...
  worker_thread_->PostTask(
      SafeTask(task_safety_.flag(), [this, packet = packet]() mutable {
        ...
        call_->Receiver()->DeliverRtpPacket(
            ...
            absl::bind_front(
                &WebRtcVoiceReceiveChannel2::MaybeCreateDefaultReceiveStream,
                this));
      }));
}

bool WebRtcVoiceReceiveChannel2::MaybeCreateDefaultReceiveStream(
    const webrtc::RtpPacketReceived& packet) {
...
bool WebRtcVoiceReceiveChannel2::GetStats(VoiceMediaReceiveInfo* info,
...
void WebRtcVoiceReceiveChannel2::FillReceiveCodecStats(
...
void WebRtcVoiceReceiveChannel2::SetRawAudioSink(
...
void WebRtcVoiceReceiveChannel2::SetDefaultRawAudioSink(
...
std::vector<webrtc::RtpSource> WebRtcVoiceReceiveChannel2::GetSources(
    uint32_t ssrc) const {
  auto it = recv_streams_.find(ssrc);
  if (it == recv_streams_.end()) {
...
void WebRtcVoiceReceiveChannel2::SetDepacketizerToDecoderFrameTransformer(
    uint32_t ssrc,
    rtc::scoped_refptr<webrtc::FrameTransformerInterface> frame_transformer) {
  RTC_DCHECK_RUN_ON(worker_thread_);
  if (ssrc == 0) {
...
bool WebRtcVoiceReceiveChannel2::MaybeDeregisterUnsignaledRecvStream(
    uint32_t ssrc) {
  RTC_DCHECK_RUN_ON(worker_thread_);
