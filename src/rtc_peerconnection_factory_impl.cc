...
namespace libwebrtc {
	...
	scoped_refptr<RTCPeerConnection> RTCPeerConnectionFactoryImpl::Create(const RTCConfiguration& configuration, scoped_refptr<RTCMediaConstraints> constraints) {
		scoped_refptr<RTCPeerConnection> peerconnection = scoped_refptr<RTCPeerConnectionImpl>(
			new RefCountedObject<RTCPeerConnectionImpl>(configuration, constraints, rtc_peerconnection_factory_));
		peerconnections_.push_back(peerconnection);
		return peerconnection;
	}

	void RTCPeerConnectionFactoryImpl::Delete(scoped_refptr<RTCPeerConnection> peerconnection) {
	...
	scoped_refptr<RTCDesktopDevice> RTCPeerConnectionFactoryImpl::GetDesktopDevice() {
		...
		{
			desktop_device_impl_ = scoped_refptr<DesktopDeviceImpl>(new RefCountedObject<DesktopDeviceImpl>());
		}
		return desktop_device_impl_;
	}


	scoped_refptr<RTCAudioSource> RTCPeerConnectionFactoryImpl::CreateAudioSource(const char* audio_source_label) {
...
*/
	...
	scoped_refptr<RTCVideoSource> RTCPeerConnectionFactoryImpl::CreateVideoSource_s(
		...
		RTCMediaConstraintsImpl* media_constraints = static_cast<RTCMediaConstraintsImpl*>(constraints.get());
		rtc::scoped_refptr<webrtc::VideoTrackSourceInterface> rtc_source_track =
			rtc_peerconnection_factory_->CreateVideoSource(std::unique_ptr<cricket::VideoCapturer>(capturer_impl->video_capturer()), media_constraints);
		scoped_refptr<RTCVideoSourceImpl> source = scoped_refptr<RTCVideoSourceImpl>(new RefCountedObject<RTCVideoSourceImpl>(rtc_source_track));
		return source;
	}
