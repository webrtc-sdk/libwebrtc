...
std::map<uint32_t, uint32_t> _sendCounts;
rtc::Buffer _makeIV(uint32_t ssrc, uint32_t timestamp) {
  rtc::Buffer iv = rtc::Buffer(IV_LENGTH);
  rtc::ArrayView<uint32_t> ivView = rtc::ArrayView<uint32_t>(
      reinterpret_cast<uint32_t*>(iv.data()), IV_LENGTH / 4);

  if (!_sendCounts[ssrc]) {
    // TOOD:
    //_sendCounts[ssrc] = Math.floor(Math.random() * 0xFFFF);
  }

  uint32_t sendCount = _sendCounts[ssrc];
...
uint8_t GetUnencryptedBytes(webrtc::TransformableFrameInterface* frame,
...
void FrameCryptorTransformer::encryptFrame(
    std::unique_ptr<webrtc::TransformableFrameInterface> frame) {
  uint8_t unencrypted_bytes = GetUnencryptedBytes(frame.get(), type_);
...
void FrameCryptorTransformer::decryptFrame(
  ...
  if (ivLength != IV_LENGTH && keyIndex != 0) {
    if (sink_callback_)
      sink_callback_->OnTransformedFrame(std::move(frame));
    return;
  }
  ...
  for (size_t i = unencrypted_bytes; i < date_in.size() - ivLength - 2; i++) {
    encrypted_payload[i - unencrypted_bytes] = date_in[i];  // ^ 0x55;
  }

  uint8_t decbuffer[65535];
  ...
  rtc::Buffer payload;
  payload.AppendData(decbuffer, declen);
  rtc::Buffer data_out;
  data_out.AppendData(frameHeader);
