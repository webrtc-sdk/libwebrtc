...
// msdk_strcopy is intended to be used with 2 parmeters, i.e. msdk_strcopy(dst,
// src) for _tcscpy_s that's possible if DST is declared as: TCHAR DST[n];
...
#define MSDK_WAIT_INTERVAL                              \
  MSDK_DEC_WAIT_INTERVAL + 3 * MSDK_VPP_WAIT_INTERVAL + \
      MSDK_ENC_WAIT_INTERVAL  // an estimate for the longest pipeline we have in
                              // samples
...
#define MSDK_MEMCPY_BITSTREAM(bitstream, offset, src, count)              \
  memcpy_s((bitstream).Data + (offset), (bitstream).MaxLength - (offset), \
           (src), (count))
...
enum class DumpType : int { MFX_DEC, MFX_VPP, MFX_ENC };
